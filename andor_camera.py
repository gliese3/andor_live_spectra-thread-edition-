...
class AndorCamera(): #
    ...
    def __init__(self, camindex=1):
        ...
        if platform.system() == "Linux":
            ...
            #dllname = "C:\\Program Files\\Andor SOLIS\\atmcd32d"
            #dllname = "C:\\Program Files\\Andor Technology\\atmcd32d"
            #dllname = "C:\\Program Files\\Andor iXon\\atmcd32d"
            dllname = "lib/atmcd64d_legacy.dll"
            print(dllname)
            self._dll = windll.LoadLibrary(dllname)
        else:
            if verbose: print("Cannot detect operating system, wil now stop")
            raise

        tekst = c_char()
        self._verbosity    = True
        print("Number of availiable cameras ", self.GetAvailableCameras())
        self.SetCurrentCamera(cameraindex=camindex)
        #tekst = c_char_p("C:\\Program Files (x86)\\Andor Technology\\")
        error = self._dll.Initialize(byref(tekst))
        if verbose: print("Initializing: %s" % ( ERROR_CODE[error]))
        ...
        # Initiate parameters
        #****
        self.image_width        = cw.value
        self.image_height       = ch.value
        #****
    ...
    def _Verbose(self, error):
        ...
        if self._verbosity is True:
            if verbose: print("[%s]: %s" % (self.FUNC(1), error))
...
# Get Camera properties
    def GetAvailableCameras(self):
        totalcameras = c_long()
        error =  self._dll.GetAvailableCameras(byref(totalcameras))
        self.N_cameras = totalcameras.value
        self._Verbose(ERROR_CODE[error] )
        return self.N_cameras
    def GetCurrentCamera(self):
        cur_camera_handler = c_long()
        error =  self._dll.GetCurrentCamera(byref(cur_camera_handler))
        self.camera_handler = cur_camera_handler.value
        self._Verbose(ERROR_CODE[error] )
        return self.camera_handler
    def SetCurrentCamera(self, cameraindex=1):
        cameraHandler = c_long()
        error =  self._dll.GetCameraHandle(c_long(cameraindex), byref(cameraHandler))
        print("cameraHandler.value = ",cameraHandler.value)
        self._Verbose(ERROR_CODE[error] )
        error =  self._dll.SetCurrentCamera(cameraHandler)
        self._Verbose(ERROR_CODE[error] )
...
# Cooler and temperature
    ...
    def GetTemperature(self):
        ...
        self._Verbose(ERROR_CODE[error] )
        #if verbose: print "Temperature is: %g [Set T: %g]" \
...
###### Single Parameters Set ######
    ...
    def SetNumberKinetics(self, numKin):
        ...
        self._Verbose(ERROR_CODE[error] )
    def SetNumberPrescans(self, numPrescans):
        error = self._dll.SetNumberPrescans(numPrescans)
        self._Verbose(ERROR_CODE[error] )
    def SetVSAmplitude(self, VSAmpl):
        error = self._dll.SetVSAmplitude(VSAmpl)
        self._Verbose(ERROR_CODE[error] )
    ...
    def SetReadMode(self, mode):
        ...
        self._ReadMode = mode
        self._Verbose(ERROR_CODE[error] )
    def SetSingleTrack(self,row,height,STHbin):
        error = self._dll.SetSingleTrack(row,height)
        self.SetSingleTrackHBin(STHbin)
        self.image_width = self._width/STHbin
        self.image_height = 1
        #self._ReadMode = mode
        self._Verbose(ERROR_CODE[error] )
    def SetSingleTrackHBin(self,STHbin):
        error = self._dll.SetSingleTrackHBin(STHbin)
        #self._ReadMode = mode
        self._Verbose(ERROR_CODE[error] )
...
###### Single Parameters Get ######
    ...
    def GetMostRecentImage(self, imageArray):
        print("self.image_width self.image_height", self.image_width, self.image_height)
        dim = self.image_width*self.image_height
        if verbose: print("Dim is %s" % dim)
        cimageArray = c_int * dim
        cimage = cimageArray()
        error = self._dll.GetMostRecentImage(pointer(cimage), dim)
        self.status = ERROR_CODE[error]
        self._Verbose(ERROR_CODE[error] )
        return cimage
        """
        #imageArray_new = numpy.zeros((0))
        for i in range(len(cimage)):
            imageArray.append(cimage[i])
            #imageArray_new+= cimage[i]
            #imageArray_new = numpy.append(imageArray_new, cimage[i])

        self._imageArray = imageArray[:]
        self._Verbose(ERROR_CODE[error] )
        #print "bla-bla", mean(imageArray_new)
...
###### Single Parameters Get/Set ######
    ...
    def SetHSSpeed(self, index):
        ...
        '''
        #try: 
        error = self._dll.SetHSSpeed(0,index)
        self._Verbose(ERROR_CODE[error] )
        self._hsspeed = index
...
###### iDus interaction Functions ######
    ...
    def SetSingleImage(self):
        ...
        if verbose: print("Width: %d Height: %d" % (self._width, self._height))
    ...
    def GetAcquisitionTimings(self):
        ...
        if verbose: print("self._exposure  = ", self._exposure)
        self._accumulate = accumulate.value
        if verbose: print("self._accumulate  = ", self._accumulate)
        self._kinetic = kinetic.value
        if verbose: print("self._kinetic  = ", self._kinetic)
        self._Verbose(ERROR_CODE[error] )

###### other functions######
    def SendSoftwareTrigger(self):
	    error = self._dll.SendSoftwareTrigger()
	    self._Verbose(ERROR_CODE[error])
###### Misc functions ######
    def SetIsolatedCropMode(self, active, cropheight, cropwidth, vbin, hbin):
        ...
        """
        error = self._dll.SetIsolatedCropMode(active, cropheight, cropwidth, vbin, hbin)
        self._Verbose(ERROR_CODE[error] )
    def SetImage(self, hbin, vbin, hstart, hend, vstart, vend):
        ...
        Output:
            None
        '''
        hend = hend - (1+hend-hstart)%hbin
        vend = vend - (1+vend-vstart)%vbin
        
        self.image_width        = (hend - hstart + 1)/hbin
        self.image_height       = (vend - vstart + 1)/vbin
    ...
    def SaveAsBmpNormalised(self, path):
        ...
        if verbose: print(maxIntensity, minIntensity)
        for i in range(len(self._imageArray)):
            (row, col) = divmod(i, self._width)
